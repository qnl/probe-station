'''module to control the probe station movement for the QNL probe station in fab.
...
class P200L():
    ...
    def reconnect(self):
        '''Reconnect to the server. Use if the server is restarted
        or the connection somehow else went bad.'''
        self.socket.close()
        self.socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.socket.connect((self.ip, self.port))
    ...
    def send_command(self, message):
        ...
        Returns: Each response may be interpreted as shown in the probe station manual'''
        try:
    ...
    def get_die(self):
        '''return the index x y values of the current die
        '''
    ...
    def goto_subsite(self, subsite):
        ...
        Args:
            subsite: index of the subsite to goto.
                current subsite x,y coordinate visible
                in P200L Navigator'''
        self.send_command(f':SUB:GOTO {subsite}')

    def goto_next_subsite(self):
        '''Move to the next subsite.
        Returns: new subsite index. returns -1 if there there
            no more subsites'''
    ...
    def use_pattern_recognition(self, do_rec=None):
        '''get/set using pattern recognition. If True, every time the stage moves to
        a new die, the probe station will use pattern recognition for local alignment'''
    ...
    def load_wafer_file(self, wafer_file):
        '''Load a wafer map file to the probe station. see manual for
        file data structure
        Args: wafer_file: file name to load
        '''
        self.send_command(f':WFR:FILE {subsite_file}')

    def load_subsite_file(self, subsite_file):
        '''Load a subsite file to the probe station. see manual for
        file data structure
        Args: wafer_file: file name to load
        '''
    ...
    def wafer_array_shape(self):
        '''Return the shape of the wafer array. returns as (col, row).'''
    ...
    def wafer_array_shape(self):
        '''Return the size of the subsite array.'''
